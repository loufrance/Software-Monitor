...
<head>
    ...
    <style>
        ...
        .btn-add { background-color: #d4a017; color: black; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 0.8em; margin-left: 10px; font-weight: bold; transition: background 0.3s; }
        .btn-add:hover { background-color: #ffd60a; }
        .intune-cell { white-space: nowrap; }
        .intune-version { margin-right: 8px; }
    </style>
</head>
<body>
    <div class="container">
...
"@
    ...
    foreach ($R in $GlobalResults) {
        ...
        $IntuneBadgeClass = "status-" + $R.INTUNESTATUS
        $IntuneZelle = "<span class='intune-version'>$($R.INTUNE)</span><span class='status-badge $IntuneBadgeClass'>$($R.INTUNESTATUS)</span>"
        ...
        $Html += @"
            <tr>
                ...
                <td>$($R.IST)</td>
                <td>$($R.AKTUELL)</td>
                <td class="intune-cell">$IntuneZelle</td>
                <td>$StatusZelle</td>
            </tr>
"@
    }
